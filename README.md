...
3、带迭代策略的迭代器：
  ...
  在实际开发，!',{,J于经常被访问的·些数据，丁以使川缓存，把这些数据存
放在内存「！，。fll．是不同的业务功能t衍要访问的数据是不同的，还有不同｛’内业务访
问权限能访问的数据也是不同的，对J：这种情况，就，l了以使川实现过滤功能的迭
代器，让不｝司功能使川不｝司的迭代器来访问。当然，这种情况也．'了以结合策略模
式来实现。
在实现过滤功能的迭代器‘l,，又有l丙种常见的而要过滤的个．，况，一种是对
数据进行整条过滤，比如只能杳石自己部门的数据：另外·种寸！'r；兄足义」数据进行
```
