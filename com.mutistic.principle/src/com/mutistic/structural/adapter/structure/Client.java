...
public class Client {
	public static void main(String[] args) {
		PrintUtil.one("适配器模式 Adapter Pattern：本质：");
		PrintUtil.two("定义", "将一个类的接口转换客户端希望的另外一个接口。适配器模式使得原本由于接口不兼容而能一起工作的那些类可以一起工作。");
		
		Existing existing = new Existing();
		PrintUtil.one("Client：客户端，调用Target接口信息");
		Target target = new Adapter(existing);
		target.test();
		
		PrintUtil.one("Client：客户端，调用适配类接口信息");
