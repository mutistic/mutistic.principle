...
```
扩展1：权限管理：
  几乎所有的权限系统都分成两个部分，一个是授权部分，一个是验证部分，
为了解它们，泞先解释两个基本的名词，安个实亻本和权限。
安全实体：就是被权限系统检测的对象，比如工资数据。
权限：就是需要被皎验的权限对象，比如查看、修改等。
安个实亻本和权限通要一起扌凸述才有意义，比如有这么个捐述：“现在要
检测登录人员对工资数据是吉有查看的权限"
“工资数''这个安个实亻本和
“了，看"这个权限啶要．起描述。如果只以现安个实亻本扌凸述，那就变成这样
“现在要检氵则登录人员对工资数"，对工资数什么呀，没有后半截，、看
就知道不完当然只有权限描述也不行，那就变成：“现在要检氵则登录人员是
古有查．石的权限”
，对ifi．的查石权限啊，也不完整。所以安个实体和权限通要
一起描述
```
