...
public class ElementExpression extends ReadXmlExpression {
	...
	public String[] interpret(Context ctx) {
		...
		PrintUtil.two("ElementExpression.interpret()：元素非终结符解释器的开始具体解释操作", preEle);
		
		// 获取根元素
		if(preEle == null) {
			ctx.setPreEle(ctx.getDocument().getDocumentElement());
			PrintUtil.three("ElementExpression：获取根元素：", ctx.getPreEle().getNodeName());
		}
		// 根据父级元素和元素名称获取当前元素，并设置回到上下文中
		else {
			ctx.setPreEle(ctx.getNowEle(preEle, eleName));
			PrintUtil.three("ElementExpression：根据父级元素和元素名称获取当前元素，并设置回到上下文中", ctx.getPreEle().getNodeName());
		}
